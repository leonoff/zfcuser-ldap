...
namespace ZfcUserLdap\Authentication\Storage;
...
class Db implements Storage\StorageInterface, ServiceManagerAwareInterface
{
    ...
    public function read()
    {
        ...
        $identity = $storedIdentity = $this->getStorage()->read();

        if (is_int($identity) || is_scalar($identity)) {
            $identity = $this->getMapper()->findById($identity);
            if (!$identity) {
                $mapper = $this->getServiceManager()->get('zfcuser_ldap_user_mapper');
                $identity = $mapper->findById($storedIdentity);
            }
        }
